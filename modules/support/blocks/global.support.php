...
if (! nv_function_exists('support')) {
  
    
    function support($block_config)
    {
        global $nv_Cache, $db_config, $global_config, $site_mods, $module_info, $module_name, $module_file, $module_data, $lang_global, $catid, $home;

        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/support/global.support.tpl')) {
            $block_theme = $global_config['module_theme'];
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/support/global.support.tpl')) {
        ...
        $xtpl = new XTemplate('global.support.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/support');
                ...
                $xtpl->assign('PACORP_SUPPORT', $item);
                 ...
                 if (! empty($item['email'])) { $xtpl->parse('main.support.email');}
                  if (! empty($item['skype'])) { $xtpl->parse('main.support.skype');}
                   if (! empty($item['facebook'])) { $xtpl->parse('main.support.facebook');}
