...
if (! function_exists('nv_product_block')) {
    /**
     * nv_product_center()
     *
     * @return
     */
    function nv_product_block($block_config)
    {
        ...
        $xtpl = new XTemplate('block.product_block.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
        $xtpl->assign('LANG', $lang_module);
        $xtpl->assign('THEME_TEM', NV_BASE_SITEURL . 'themes/' . $module_info['template']);
        $xtpl->assign('WIDTH', $pro_config['homewidth']);
        $xtpl->assign('NUMVIEW', $num_view);

        $cache_file = NV_LANG_DATA . '_block_module_product_block_' . NV_CACHE_PREFIX . '.cache';
...
$content = nv_product_block($block_config);
